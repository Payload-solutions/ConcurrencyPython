...
if __name__ == "__main__":
    ...
    pprint("Creando un %d x %d malla de procesadores..." % (mpi_rows, mpi_cols) )
    ...
    pprint(78*"=")
    pprint("Computarizado (serial) %d x %d x %d en  %6.2f segundos" % (my_M, my_M, my_N, t_serial))
    pprint(" ... Esperando computo paralelo para tomar %6.2f segundos" % (mpi_rows*mpi_rows*mpi_cols*t_serial / comm.size))
    pprint("Computarizado (Parallel) %d x %d x %d en  %6.2f segundos" % (mpi_rows*my_M, mpi_rows*my_M, mpi_cols*my_N, t_total))
